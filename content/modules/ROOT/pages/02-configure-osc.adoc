...
----
+
IMPORTANT: There is a difference between `AZURE_INSTANCE_SIZE` and `AZURE_INSTANCE_SIZES`. The former is used to define the default instance size, if not specified in the `io.katacontainers.config.hypervisor.machine_type` xref:03-deploy-workload.adoc#optional-options[pod annotation].

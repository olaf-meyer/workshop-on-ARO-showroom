...
----
+
NOTE: Currently, only a route with a valid CA-signed certificate is supported. You cannot use a route with self-signed certificate. In ARO, the above route seems not to work anymore. If you are trying this guide from an Azure self managed cluster, there is the `NodePort` option available, which consists of adding `kbsServiceType: NodePort` under `spec` in the xref:02-configure-trustee.adoc#trustee-kbsconfig[Trustee KbsConfig] (restart trustee deployment), and then set `TRUSTEE_HOST` to `http://your_worker_node_ip_where_trustee_is:kbs_service_nodeport`. Using nodeport means using unsecure http, and therefore also xref:02-configure-osc.adoc#pp-initdat[initdata] has to be configured to support it. This is not a supported approach for production and just useful for trying everything in a single cluster. For the purpose of this workshop, we will enable NodePort.
+
...
**Purpose of this resource**: If your TEE is **Intel Trust Domain Extensions (TDX)**, meaning the instance size you use or plan to use (it's in `AZURE_INSTANCE_SIZES` list) is Standard_DC**e***, you must configure the Provisioning Certificate Caching Service (PCCS). The PCCS retrieves Provisioning Certification Key (PCK) certificates and caches them in a local database.
